...
- [x] 새 게임 생성(다리 정보 전달)
- [x] 사용자 입력(이동할 위치) 받음
- [x] move 실행(GameRound 내부의 위치 이동 메서드 실행)
- [x] 정답 여부, 위치 return받음
- [x] 오답이면 종료(while문 끝) 후 재시도 여부 입력 받음
- [x] 정답이고 위치 아직 길이와 일치 안하면 이동
- [x] 정답이고 위치 길이와 일치하면 종료(while문 끝)
- [x] 종료 시 tryCound ++
- [x] 재시도 시 retry 로직 실행 -> 메인 로직 호출
- [x] tryCount, 결과 받아서 출력
