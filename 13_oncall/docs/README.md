...
- [ ] Organizer 생성
  ...
  - [ ] 저장
- [ ] 결과 생성
  - [ ] Manager에 Organizer, Calendar 전달하여 생성
  - [ ] Manager 실행
  - [ ] 결과 받아옴
- [ ] 결과 출력
  - [ ] 결과 가공
...
- [ ] initCalendar(월, 요일) 실행
  - [ ] 해당 월이 며칠인지 저장
  - [ ] 요일에 따라 정보에 {일, 요일, 주말여부} push
- [ ] 휴일 처리
  - [ ] 해당 월의 휴일 정보 가져옴
  - [ ] 그 일에 해당되는 정보 가져옴
  - [ ] 주말이면 pass
...
- [ ] 평일 로직
  - [ ] 평일 shift
  - [ ] shift한 내용이 전날 근무한 사람이랑 같은지 확인
    - [ ] 같으면 추가 shift해서 그 사람 근무자 처리
    - [ ] 아까 shift한 내용은 unshift해서 다시 넣음
  - [ ] 근무자 이름 변경
  - [ ] 평일 push
- [ ] 주말 로직(평일과 동일함)
...
- [ ] 총 며칠인지(Calendar) 통해 가져옴
- [ ] 반복문 생성(main로직)
- [ ] 반복문 내부에 들어가는 로직
  - [ ] 해당 일이 주말인지 & 휴일인지 확인
  - [ ] 주말이면 Organizer 주말로직, 평일이면 평일로직
  - [ ] 결과 받아서
  - [ ] 휴일이면 요일에 (휴일) 추가
  - [ ] 결과 {일, 요일, 이름} push
- [ ] 결과 return
